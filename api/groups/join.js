...
export default async function handler(req, res) {
  ...
  if (!user_id||!school_id||!grade||!display_name)
    return res.status(400).json({ error:"Pflichtfelder fehlen" });
  try {
    let [group] = await sql`
      SELECT * FROM groups
      WHERE school_id=${school_id} AND grade=${grade}
        AND section=${section||null} AND school_year='2025/26'
    `;
    if (!group) {
      [group] = await sql`
    ...
    let [member] = await sql`
      SELECT * FROM members WHERE user_id=${user_id} AND group_id=${group.id}
    `;
    const isFirst = group.member_count === 0;
    if (!member) {
      [member] = await sql`
        INSERT INTO members(user_id,group_id,display_name,children,status,is_admin)
        VALUES(${user_id},${group.id},${display_name},
          ${JSON.stringify(children||[])},
          ${isFirst?"active":"pending"},${isFirst})
        RETURNING *
      `;
    }
    const [school] = await sql`SELECT name FROM schools WHERE id=${school_id}`;
    res.status(200).json({
      group: { ...group, school_name: school?.name },
      member
    });
  } catch(e) {
    console.error(e);
