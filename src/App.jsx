...
export default function App() {
  ...
  const handleJoined = (result) => {
    // Neue Mitgliedschaft zur Liste hinzufügen
    setMemberships(prev => {
  ...
  return (
    <Routes>
      ...
      <Route path="/*" element={
        ...
        <Feed user={user} memberships={memberships} onLogout={handleLogout} onAddChild={() => window.location.href="/join"} />
      }/>
    </Routes>
  );
}
    ...
    <div style={{
      ...
      <div style={{position:"relative", marginBottom:20}}>
        ...
        <div style={{
          ...
          <span style={{fontFamily:"Georgia,serif",fontSize:52,fontWeight:900,color:"#0D0D1A",lineHeight:1,marginTop:4}}>K</span>
        </div>
      </div>
      <div style={{textAlign:"center",animation:"fadeIn 0.5s ease 0.6s both"}}>
        <div style={{fontFamily:"Georgia,serif",fontSize:36,fontWeight:900,color:"#fff",letterSpacing:"-0.03em",marginBottom:8}}>Klassly</div>
        <div style={{fontFamily:"system-ui,sans-serif",fontSize:14,color:"rgba(255,255,255,0.38)",letterSpacing:"0.04em"}}>für Eltern mit SchuKis</div>
      </div>
      <div style={{display:"flex",gap:8,marginTop:40,animation:"fadeIn 0.5s ease 0.8s both"}}>
        {[0,1,2].map(i=>(
          <div key={i} style={{width:7,height:7,borderRadius:"50%",background:"rgba(61,214,200,0.6)",animation:`blink 1.4s ease-in-out ${i*0.22}s infinite`}}/>
        ))}
      </div>
      <div style={{position:"absolute",bottom:24,fontFamily:"system-ui,sans-serif",fontSize:11,color:"rgba(255,255,255,0.18)",letterSpacing:"0.12em",animation:"fadeIn 0.5s ease 1s both"}}>2026</div>
      <style>{`
        @keyframes spinPulse{0%{transform:rotate(0deg) scale(1)}70%{transform:rotate(360deg) scale(1)}82%{transform:rotate(360deg) scale(1.13)}91%{transform:rotate(360deg) scale(0.96)}100%{transform:rotate(360deg) scale(1)}}
        @keyframes ringOut{0%{transform:scale(1);opacity:0.6}100%{transform:scale(1.9);opacity:0}}
